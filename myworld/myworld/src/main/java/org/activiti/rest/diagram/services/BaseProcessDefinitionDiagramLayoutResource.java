...
public class BaseProcessDefinitionDiagramLayoutResource {
  ...
  public ObjectNode getDiagramNode(String processInstanceId, String processDefinitionId) {
    ...
    if (processInstanceId != null) {
      processInstance = engineService.getRuntimeService().createProcessInstanceQuery().processInstanceId(processInstanceId).singleResult();
      if (processInstance == null) {
        throw new ActivitiObjectNotFoundException("Process instance could not be found");
      }
      processDefinitionId = processInstance.getProcessDefinitionId();

      List<ProcessInstance> subProcessInstances = engineService.getRuntimeService()
      ...
      for (ProcessInstance subProcessInstance : subProcessInstances) {
        ...
        String superExecutionId = ((ExecutionEntity) subProcessInstance)
            .getSuperExecutionId();
        ProcessDefinitionEntity subDef = (ProcessDefinitionEntity) engineService.getRepositoryService().getProcessDefinition(subDefId);
    ...
    ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) engineService.getRepositoryService().getProcessDefinition(processDefinitionId);
    ...
    if (processInstance != null) {
      ...
      highLightedActivities = engineService.getRuntimeService().getActiveActivityIds(processInstanceId);
  ...
  private List<String> getHighLightedFlows(String processInstanceId, ProcessDefinitionEntity processDefinition) {
    ...
    List<HistoricActivityInstance> historicActivityInstances = engineService.getHistoryService()
    ...
    List<String> highLightedActivities = engineService.getRuntimeService().getActiveActivityIds(processInstanceId);
  ...
  private void getActivity(String processInstanceId, ActivityImpl activity, ArrayNode activityArray, 
    ...
    if ("callActivity".equals(properties.get("type"))) {
      ...
      if (callActivityBehavior != null) {
        ...
        if (processInstance != null) {
          List<Execution> executionList = engineService.getRuntimeService().createExecutionQuery()
              .processInstanceId(processInstanceId)
        ...
        if (processInstanceArray.size() == 0 && StringUtils.isNotEmpty(callActivityBehavior.getProcessDefinitonKey())) {
          ...
          ProcessDefinition lastProcessDefinition = engineService.getRepositoryService()
  ...
  private boolean isGraphicNotationDefined(ProcessDefinitionEntity processDefinition) {
    return ((ProcessDefinitionEntity) engineService.getRepositoryService()
